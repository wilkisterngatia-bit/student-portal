...
class _ResultsScreenState extends State<ResultsScreen> {
  ...
  Widget _buildResultsInsight() {
    ...
    final weakest = _results.reduce((a, b) => a.total < b.total ? a : b);

    if (weakest.total < 50) {
      return AiInsightCard(
        tone: InsightTone.warning,
        message:
            '${weakest.unitCode} is currently below the pass mark (${weakest.total}%). Consider registering for a retake under Exam registration.',
      );
    }
    if (weakest.total < 60) {
      return AiInsightCard(
        tone: InsightTone.info,
        message: '${weakest.unitCode} is your weakest unit this semester at ${weakest.total}%. A bit more revision here could lift your overall grade.',
      );
    }
